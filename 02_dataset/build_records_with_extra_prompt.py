...
def _prepend_extra_prompt(record: dict, extra_prompt: str) -> dict:
    if not extra_prompt:
        return record
    out = dict(record)
...
def main() -> None:
    ...
    ap.add_argument("--extra_prompt_path", type=str, required=True)
    ...
    extra_prompt = _load_extra_prompt(args.extra_prompt_path)
    ...
    print(
        json.dumps(
            {
                "sft_rows": len(sft_records),
                "rl_rows": len(rl_records),
